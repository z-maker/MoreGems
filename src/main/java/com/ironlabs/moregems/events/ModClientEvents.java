package com.ironlabs.moregems.events;
...
@Mod.EventBusSubscriber(modid = MoreGems.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE, value = Dist.CLIENT)
    ...
    public static void onCraftingTableOpen(GuiOpenEvent event) {
        if (event.isCancelable()) {
            if (event.getGui() instanceof CraftingScreen) {
                ...
                MoreGems.LOGGER.info("Player tried to open a crafting table!");
            }
        }
    }
...
//            MoreGems.LOGGER.info("Player tried to jump with a stick!");
